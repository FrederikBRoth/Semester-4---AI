    ...
    for generation in range(num_of_generations):
        ...
        for i in range(len(population)):
            ...
            for child in children:
                sorted_by_fitness.append(fitness_fn(child))
                child_fitness_dict[fitness_fn(child)] = child
            sorted_by_fitness.append(fitness_fn(mother))
            child_fitness_dict[fitness_fn(mother)] = mother
            sorted_by_fitness.append(fitness_fn(father))
            child_fitness_dict[fitness_fn(father)] = father

        sorted_by_fitness.sort()
        top_half = sorted_by_fitness[len(sorted_by_fitness)//2:]
        for fitness in top_half:
            new_population.add(child_fitness_dict[fitness])
...
def reproduce(mother, father, queens):
    ...
    '''
    crossover = random.randint(0, queens-1)
...
def mutate(children, queens):
    ...
    for child in children:
        ...
        mutation = child[:mutatepoint] + (random.randint(1, queens),) + child[mutatepoint + 1:]
